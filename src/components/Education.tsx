...
//   // Change 1: State to track the active slide for each of the 3 education cards
//   const [activeSlides, setActiveSlides] = useState([0, 0, 0]);

//   // Animation variants for the horizontal slide effect
//   const slideVariants = {
//     enter: (direction: number) => ({ x: direction > 0 ? '100%' : '-100%', opacity: 0 }),
//     center: { zIndex: 1, x: 0, opacity: 1 },
//     exit: (direction: number) => ({ zIndex: 0, x: direction < 0 ? '100%' : '-100%', opacity: 0 }),
...
//               Formal Education
//             </h3>
//             {/* Change 2: Responsive grid for the cards */}
//             <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
//               {education.map((edu, cardIndex) => (
//                 // Change 3: The card is now a fixed-height, relative container for the slides
//                 <div key={cardIndex} className="relative h-[450px] rounded-lg overflow-hidden border border-border bg-card shadow-sm flex flex-col">
//                   <div className="flex-grow relative">
//                     <AnimatePresence initial={false} mode="wait">
//                       {/* We use the active slide state to render the correct content */}
//                       <motion.div
//                         key={activeSlides[cardIndex]}
//                         custom={1} // You can use this for direction later if needed
//                         variants={slideVariants}
//                         initial="enter"
//                         animate="center"
//                         exit="exit"
...
//                         className="absolute inset-0 p-6 flex flex-col"
//                       >
//                         {/* --- SLIDE CONTENT RENDERED CONDITIONALLY --- */}
//                         {activeSlides[cardIndex] === 0 && (
//                           <div className="space-y-2">
//                             <CardTitle className="text-xl">{edu.degree}</CardTitle>
//                             <CardDescription className="text-lg font-medium text-primary">{edu.institution}</CardDescription>
...
//                           <div className="space-y-2">
//                             <CardTitle>Description</CardTitle>
//                             <p className="text-muted-foreground text-sm flex-grow overflow-y-auto h-[280px] pr-2">{edu.description}</p>
//                           </div>
//                         )}
//                         {activeSlides[cardIndex] === 2 && (
//                           <div className="space-y-2">
//                             <CardTitle>Achievements</CardTitle>
//                             <ul className="space-y-2 flex-grow overflow-y-auto h-[280px] pr-2">
//                               {edu.achievements.map((a, i) => <li key={i} className="flex items-start gap-3"><CheckCircle2 className="w-4 h-4 text-secondary flex-shrink-0 mt-1" /><span className="text-sm text-muted-foreground">{a}</span></li>)}
//                             </ul>
//                           </div>
//                         )}
...
//                           <div className="space-y-2">
//                              <CardTitle>Relevant Coursework</CardTitle>
//                              <div className="flex flex-wrap gap-2 flex-grow overflow-y-auto h-[280px] pr-2">
//                                {edu.relevantCourses.map((c, i) => <Badge key={i} variant="outline" className="text-xs border-primary/30 text-primary/80">{c}</Badge>)}
//                              </div>
//                           </div>
//                         )}
...
//                     </AnimatePresence>
//                   </div>
//                   {/* Change 4: Navigation controls at the bottom of each card */}
//                   <div className="flex-shrink-0 p-4 border-t border-border flex items-center justify-between">
//                     <button onClick={() => handleSlideChange(cardIndex, 'prev')} className="p-2 rounded-full hover:bg-muted"><ArrowLeft className="w-4 h-4" /></button>
//                     <div className="flex items-center gap-2">
//                       {[0, 1, 2, 3].map(i => (
